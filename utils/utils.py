...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.2, merge=False, classes=None, agnostic=False):
    ...
    # Settings
    min_wh, max_wh = 0, 100  # (pixels) minimum and maximum box width and height
...
def print_mutation(hyp, results, bucket=''):
    ...
    a = '%10s' * (len(hyp)-1) % tuple(hyp.keys())[1:]  # hyperparam keys
    b = '%10.3g' * (len(hyp)-1) % tuple(hyp.values())[1:]  # hyperparam values
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        ...
        cv2.putText(img, label, (c1[0] - t_size[0], c1[1] + t_size[1]+3), 0, tl / 3, color, thickness=tf, lineType=cv2.LINE_AA)
